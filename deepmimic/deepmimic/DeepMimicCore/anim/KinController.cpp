...
void cKinController::SetTimeFromIndex(int index)
{

	int num_frames = GetNumMotionFrames()-1;
	double motion_length = GetMotionDuration();
	if (EnableMotionLoop()) {
		mTime = motion_length * (index / (double)num_frames);
	}
	else {
		double mod_index = (double)(index % num_frames);
		mTime = motion_length * (mod_index / num_frames);
	}
}
