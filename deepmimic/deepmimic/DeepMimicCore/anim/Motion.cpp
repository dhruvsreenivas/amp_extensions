...
void cMotion::CalcFrameVel(double time, cMotion::tFrame& out_vel) const
{
	...
	else
	{
		...
		CalcIndexBlend(time, idx, blend);
		/*
		If time is before first frame time, set frame 0 vel to be 0
		when computing vel. Commented out for now. 
		
		Eigen::VectorXd vel0;
		if (time < mFrameTimes(1)) {
			vel0 = Eigen::VectorXd::Zero(GetNumDof());
		}
		else {
			vel0 = vel_frame->row(idx);
		}
		*/
