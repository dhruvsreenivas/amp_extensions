...
void cMotionController::CalcIndexPose(int index, Eigen::VectorXd& out_pose) const
{
	const cMotion& motion = GetMotion();
	...
	if (EnableMotionLoop())
	{
		int mod_index = index % num_frames;
		out_pose = motion.GetFrame(mod_index);

		tVector cycle_delta = GetCycleRootDelta();

		int cycle_count = index / num_frames;
		tVector root_offset = cycle_count * cycle_delta;
		...
		out_pose = motion.GetFrame(index);

	}
}

void cMotionController::CalcIndexVel(int index, Eigen::VectorXd& out_vel) const
{
	const cMotion& motion = GetMotion();
	int num_frames = motion.GetNumFrames();
	if (!EnableMotionLoop() && index >= num_frames) {
		out_vel = Eigen::VectorXd::Zero(motion.GetNumDof());
	}
	else {
		int mod_index = index % num_frames;
		out_vel = motion.GetFrameVel(mod_index);
	}
