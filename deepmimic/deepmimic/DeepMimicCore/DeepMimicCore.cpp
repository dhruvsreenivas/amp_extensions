...
void cDeepMimicCore::Init()
{
	...
	{
		//new
		std::string deepmimic_path = "";
		mArgParser->ParseString("deepmimic_path", deepmimic_path);
		if (deepmimic_path.length() > 0 && deepmimic_path.back() != '/') {
			deepmimic_path += "/";
		}
		cDrawUtil::InitDrawUtil(deepmimic_path);
		InitFrameBuffer();
	}
	SetupScene();
}
...
void cDeepMimicCore::UpdateSyncTimestep(double timestep) {
	mScene->UpdateSyncTimestep(timestep);
}
...
void cDeepMimicCore::ResetIndex(int index, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{

	mScene->ResetIndex(index, resolve, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
	mUpdatesPerSec = 0;
}

void cDeepMimicCore::ResetTime(double time, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	mScene->ResetTime(time, resolve, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
	mUpdatesPerSec = 0;
}

double cDeepMimicCore::GetAgentUpdateRate(int agent_id) const
{
	return mScene->GetAgentUpdateRate(agent_id);
}

double cDeepMimicCore::GetMotionLength() const {
	return mScene->GetMotionLength();
}

int cDeepMimicCore::GetStatePoseOffset(int agent_id) const
{
	return mScene->GetStatePoseOffset(agent_id);
}

int cDeepMimicCore::GetStatePoseSize(int agent_id) const
{
	return mScene->GetStatePoseSize(agent_id);
}

int cDeepMimicCore::GetStateVelOffset(int agent_id) const
{
	return mScene->GetStateVelOffset(agent_id);
}

int cDeepMimicCore::GetStateVelSize(int agent_id) const
{
	return mScene->GetStateVelSize(agent_id);
}

int cDeepMimicCore::GetStatePhaseOffset(int agent_id) const
{
	return mScene->GetStatePhaseOffset(agent_id);
}

int cDeepMimicCore::GetStatePhaseSize(int agent_id) const
{
	return mScene->GetStatePhaseSize(agent_id);
}

int cDeepMimicCore::GetPosFeatureDim(int agent_id) const
{
	return mScene->GetPosFeatureDim(agent_id);
}

int cDeepMimicCore::GetRotFeatureDim(int agent_id) const
{
	return mScene->GetRotFeatureDim(agent_id);
}
...
int cDeepMimicCore::GetAMPObsPoseSize() const
{
	const auto& rl_scene = GetRLScene();
	if (rl_scene != nullptr)
	{
		return rl_scene->GetAMPObsPoseSize();
	}
	return 0;
}

int cDeepMimicCore::GetAMPObsVelSize() const
{
	const auto& rl_scene = GetRLScene();
	if (rl_scene != nullptr)
	{
		return rl_scene->GetAMPObsVelSize();
	}
	return 0;
}
...
std::vector<double> cDeepMimicCore::RecordAMPObsAgentCurrent(int agent_id)
{
	const auto& rl_scene = GetRLScene();
	if (rl_scene != nullptr)
	{
		Eigen::VectorXd obs;
		rl_scene->RecordAMPObsAgentCurrent(agent_id, obs);
...
std::vector<double> cDeepMimicCore::GetDTWBacktrackPath(int agent_id)
{
	const auto& rl_scene = GetRLScene();
	if (rl_scene != nullptr)
	{
		Eigen::VectorXd obs;
		rl_scene->GetDTWBacktrackPath(agent_id, obs);
