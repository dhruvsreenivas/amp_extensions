...
cRand::cRand()
{
	...
	/*
	New random generator added by Albert, for generating vector of doubles.Could also use
	mRandGen but mGen more random.
	*/
	mGen = std::mt19937(mRandGen());
...
cRand::cRand(unsigned long int seed) : cRand()
{
	Seed(seed);
}
...
Eigen::VectorXd cRand::RandDoubleEigen(int size, double min, double max) {
	std::uniform_real_distribution<double> distribution(min, max);
	auto lambda_gen = [&]() {return distribution(mGen); };
	return Eigen::VectorXd::NullaryExpr(size, lambda_gen);
}
