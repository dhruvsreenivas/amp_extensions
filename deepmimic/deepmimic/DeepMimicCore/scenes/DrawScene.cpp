...
void cDrawScene::ParseArgs(const std::shared_ptr<cArgParser>& parser)
{
	...
	mArgParser->ParseString("deepmimic_path", mDeepMimicPath); //new
	if (mDeepMimicPath.length()>0 && mDeepMimicPath.back() != '/') {
		mDeepMimicPath += "/";
	}
	std::cout << "In drawScene parseargs: " << mDeepMimicPath << std::endl;
...
void cDrawScene::ResetIndex(int index, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	cScene::ResetIndex(index, resolve, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
	ResetCamera();
}

void cDrawScene::ResetTime(double time, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	cScene::ResetTime(time, resolve, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
	ResetCamera();
}
...
void cDrawScene::InitRenderResources()
{
...

	{
		...
		succ &= mShaderDepth->BuildShader(mDeepMimicPath + "data/shaders/Mesh_VS.glsl", mDeepMimicPath + "data/shaders/Depth_PS.glsl");
...

	{
		...
		succ &= mShaderMesh->BuildShader(mDeepMimicPath + "data/shaders/Mesh_VS.glsl", mDeepMimicPath + "data/shaders/Lighting_Lambert_PS.glsl");
...
{
	bool succ = true;
	mGridTex = std::unique_ptr<cTextureDesc>(new cTextureDesc(mDeepMimicPath + "data/textures/grid0.png", true));
