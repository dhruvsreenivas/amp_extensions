...
double cSceneImitate::GetAgentUpdateRate(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetUpdateRate();
}

int cSceneImitate::GetStatePoseOffset(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetStatePoseOffset();
}

int cSceneImitate::GetStatePoseSize(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetStatePoseSize();
}

int cSceneImitate::GetStateVelOffset(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetStateVelOffset();
}

int cSceneImitate::GetStateVelSize(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetStateVelSize();
}

int cSceneImitate::GetStatePhaseOffset(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetStatePhaseOffset();
}

int cSceneImitate::GetStatePhaseSize(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetStatePhaseSize();
}

int cSceneImitate::GetPosFeatureDim(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetPosFeatureDim();
}

int cSceneImitate::GetRotFeatureDim(int agent_id) const
{
	const auto& ctrl = GetController(agent_id);
	const cCtController* ct_ctrl = dynamic_cast<const cCtController*>(ctrl.get());
	return ct_ctrl->GetRotFeatureDim();
}
...
void cSceneImitate::UpdateCharactersSyncTimestep(double timestep)
{
	const auto& kin_char = GetKinChar();
	...
	/*	
	* Albert - I don't think this is needed like in regular update. This is also weird as I think
	* it's out of sync. This function is supposed to update everything
	* to step t+1 (assuming we're at time t right now). However, this 
	* function aligns kin character root with sim character's position at time
	* t but in the function after this the sim character is updated to 
	* timestep t+1 so we're alredy out of sync. I'll leave this in as a comment for now. 
	if (curr_phase < prev_phase)
	{
		std::cout << "prev_phase: " << prev_phase << "\n";
		std::cout << "curr_phase: " << curr_phase << "\n";
	...
	const auto& curr_char = GetCharacter();
	curr_char->ClearJointTorques(); 
}
...
void cSceneImitate::ResetCharactersTime(double time, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	cRLSceneSimChar::ResetCharacters();

	ResetKinCharTime(time, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, 
		interp, knee_rot);
	if (EnableSyncChar())
	{
		SyncCharacters();
...
void cSceneImitate::ResetCharactersIndex(int index, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	cRLSceneSimChar::ResetCharacters();
	ResetKinCharIndex(index, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, 
		interp, knee_rot);
	if (EnableSyncChar())
	{
		SyncCharacters();
...
void cSceneImitate::ResetKinCharTime(double time, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose, 
	bool vel_noise, double interp, bool knee_rot)
{
	double rand_time = time;
	const cSimCharacter::tParams& char_params = mCharParams[0];
	const auto& kin_char = GetKinChar();
	...
	kin_char->SetTime(rand_time);
	kin_char->Pose(rand_time);
	kin_char->AddNoise(noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
...
void cSceneImitate::ResetKinCharIndex(int index, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose, 
	bool vel_noise, double interp, bool knee_rot)
{
	const cSimCharacter::tParams& char_params = mCharParams[0];
	const auto& kin_char = GetKinChar();
	...
	kin_char->SetOriginRot(tQuaternion::Identity());
	kin_char->SetOriginPos(char_params.mInitPos); // reset origin
	kin_char->SetTimeFromIndex(index);
	kin_char->PoseIndex(index);
...
double cSceneImitate::GetMotionLength() const {
	const auto& kin_char = GetKinChar();
	double dur = kin_char->GetMotionDuration();
	return dur;
}