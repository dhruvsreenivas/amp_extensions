...
void cSceneSimChar::UpdateSyncTimestep(double time_elapsed)
{
	cScene::UpdateSyncTimestep(time_elapsed);
	...
	// order matters!
	UpdateCharactersSyncTimestep(time_elapsed);
	//UpdateWorld(time_elapsed); //We don't want to actually update the world and step in the simulation
	UpdateGround(time_elapsed);
	UpdateObjs(time_elapsed);
	UpdateJoints(time_elapsed);
	...
	//ResolveCharGroundIntersect();//I don't think we should resolve every time we update. Only when resetting. 
}

void cSceneSimChar::UpdateCharactersSyncTimestep(double time_elapsed) {
}
...
void cSceneSimChar::ResetSceneIndex(int index, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	...
	ResetWorld();
	ResetCharactersIndex(index, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
	ResetGround();
	CleanObjs();
...
void cSceneSimChar::ResetSceneTime(double time, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	cScene::ResetScene();
	...
	ResetWorld();
	ResetCharactersTime(time, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
	ResetGround();
	CleanObjs();
...
void cSceneSimChar::ResetCharactersIndex(int index,  bool noise_bef_rot, double low, double high, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
}

void cSceneSimChar::ResetCharactersTime(double time,  bool noise_bef_rot, double low, double high, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot) {
