...
void cSceneImitateAMP::ResetIndex(int index, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	cSceneImitate::ResetIndex(index, resolve, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
...
void cSceneImitateAMP::ResetTime(double time, bool resolve, bool noise_bef_rot, double min, double max, double radian, bool rot_vel_w_pose,
	bool vel_noise, double interp, bool knee_rot)
{
	cSceneImitate::ResetTime(time, resolve, noise_bef_rot, min, max, radian, rot_vel_w_pose, vel_noise, interp, knee_rot);
...
void cSceneImitateAMP::RecordAMPObsAgentCurrent(int agent_id, Eigen::VectorXd& out_data)
{
	...
	/*
	* Divide size by 2 here since original amp obs contains pose + vel for curr and prev state. 
	* This function only returns pose + vel features for this state. 
	*/
	out_data = std::numeric_limits<double>::quiet_NaN() * Eigen::VectorXd::Ones(GetAMPObsSize() / 2);
...
double cSceneImitateAMP::CalcRewardTimeWarp(int agent_id) const
{
	...
	if (IsEpisodeEnd())
	{
		...
		const auto& kin_char = GetKinChar();
		/*
		Added by Albert in order to add final state to buffer
		*/
		...
		mTimeWarper->AddRemainderCost(remainder_steps * term_step_cost); //Artifically adding in remainder cost back into path
		reward = cost;
	}
	return reward;
}

void cSceneImitateAMP::GetDTWBacktrackPath(int agent_id, Eigen::VectorXd& out_data) {
	/*
	Since DTW is based on dynamic programming, we can backtrack and infer
	"costs" per state. 
	*/
	out_data = mTimeWarper->GetDTWBacktrackPath();
}
...
void cSceneImitateAMP::BuildTimeWarper()
{
	...
	if (std::isfinite(max_time))
	{
		...
		//UpdateTimeWarper(); Removed by Albert. 
	}
}
...
void cSceneImitateAMP::ResetTimeWarper()
{
	mTimeWarper->Reset();
	//UpdateTimeWarper(); Removed by Albert 
	//Don't need this as first update call will add iniital state to the time warper buffer.
	//Having this doubles the initial state
}
