...
cCtController::cCtController() : cDeepMimicCharController()
{
	...
	mRecordAllWorld = false; //new
	mRecordVelAsPos = false; //new
	SetViewDistMax(1);
...
bool cCtController::ParseParams(const Json::Value& json)
{
	...
	mRecordAllWorld = json.get("RecordAllWorld", mRecordAllWorld).asBool();
	mRecordVelAsPos = json.get("RecordVelAsPos", mRecordVelAsPos).asBool();
	return succ;
}
...
void cCtController::BuildStateVel(Eigen::VectorXd& out_vel) const
{
	...
	for (int i = 0; i < num_parts; ++i)
	{
		...
		if (mRecordVelAsPos) {
			out_vel.segment(vel_idx, vel_dim) = curr_vel.segment(0, vel_dim) * dt;
		}
		else {
		...
		if (mRecordVelAsPos) {
			out_vel.segment(ang_vel_idx, ang_vel_dim) = curr_ang_vel.segment(0, ang_vel_dim) * dt;
		}
		else {
