...
class MLP:
    def __init__(self, input_size, output_size,
                 ...
                 seed=None):
        """
        :param input_size: dimension of input (state size)
        :param output_size: dimension of ouptut (action size)
        :param hidden_sizes: network hidden layer sizes (currently 2 layers only)
        :param min_log_std: log_std is clamped at this value and can't go below
        :param init_log_std: initial log standard deviation
        :param eps: probability of sampling random action 
        :param seed: random seed
        """
        self.n = input_size  # number of states
        self.m = output_size  # number of actions
    ...
    def get_action(self, observation):
        ...
        if np.random.uniform() < self.eps:
            action = np.random.uniform(0, 1, self.m)
        else:
