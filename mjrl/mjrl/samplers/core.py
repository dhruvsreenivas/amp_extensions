...
def sample_paths(
    ...
    if num_cpu == 1:
        ...
        return rollout_func(**input_dict)

    # do multiprocessing otherwise
    paths_per_cpu = int(np.ceil(num_traj/num_cpu))
    input_dict_list= []
    for i in range(num_cpu):
        inputs = (paths_per_cpu, env, policy, eval_mode, horizon, base_seed+i*paths_per_cpu, env_kwargs)
        ...
        input_dict_list.append(inputs)
    if suppress_print is False:
        start_time = timer.time()
        print("####### Gathering Samples #######")

    #results = _try_multiprocess(rollout_func, input_dict_list,
    #                            num_cpu, max_process_time, max_timeouts)
    results = _try_torch_multiprocess(rollout_func, input_dict_list, num_cpu)
    paths = []
...
def _try_torch_multiprocess(func, inputs, num_cpu):
    with torch_mp.Pool(processes=num_cpu) as pool:
