...
def train_agent(job_name, agent,
    ...
    mean_pol_perf = 0.0
    e = GymEnv(agent.env.env.spec.id)
    ...
    for i in range(i_start, niter):
        ...
        args = dict(N=N, sample_mode=sample_mode, gamma=gamma, gae_lambda=gae_lambda, num_cpu=num_cpu, reward_kwargs=reward_kwargs)
        # NOTE: Samples are inputed here
        ...
        if save_freq != 0 and i > 0 and i % save_freq == 0:
            if agent.save_logs:
        ...
        if not adroit:
            ...
            print("[ %s ] %4i %5.2f %5.2f %5.2f %5.2f" % (timer.asctime(timer.localtime(timer.time())),
                                                 i, train_curve[i], mean_pol_perf, best_perf, stats[-1]))
